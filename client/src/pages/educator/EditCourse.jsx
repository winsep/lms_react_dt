...
const EditCourse = () => {
  ...
  // Fetch course data from backend when component mounts
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        if (data.success) {
          ...
          setImagePreviewUrl(course.courseThumbnail);
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (data.success) {
        ...
        navigate("/educator/my-course"); // hoặc đường dẫn thực tế tới trang MyCourse
      } else {
        toast.error(data.message);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0">
        ...
        <div className="flex items-center justify-between flex-wrap">
          ...
          <div className="flex md:flex-row flex-col items-center gap-3">
            ...
            <label htmlFor="thumbnailImage" className="flex items-center gap-3">
              ...
              <img
                className="max-h-10"
                src={image ? URL.createObjectURL(image) : imagePreviewUrl}
                />
      ...
      <button
        ...
        className="bg-black text-white w-max py-2.5 px-8 rounded mt-2 mb-4 cursor-pointer"
      >
        ← Back to My Courses
      </button>
    </div>
